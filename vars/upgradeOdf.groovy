    ...
    script {
        ...
        try {
            sh '''
               ...
               ssh -o 'StrictHostKeyChecking no' -i ${WORKSPACE}/deploy/id_rsa root@${BASTION_IP} "odf_csv=`/usr/local/bin/oc get csv -n openshift-storage |grep odf-operator | awk {'print $1'}`; /usr/local/bin/oc get csv $odf_csv -n openshift-storage -o yaml |grep full_version |awk {'print $2'} > odf-full-build.txt"
